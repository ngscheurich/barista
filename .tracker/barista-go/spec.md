...
A faithful port of Barista from Gleam/BEAM to Go, preserving observable behavior. See ADR-0001 (Go rewrite), ADR-0002 (TOML + Mustache), and ADR-0004 (theme vs flavor).
...
`<theme>` is the directory name of a theme under the themes directory. Exactly one positional argument, zero unnamed args. Help text: `Serves up a new <theme> for your terminal apps.` Cobra provides `--help` and the standard flag handling.
...
| Themes dir | `<config dir>/themes` | — | No (assumed to exist) |
...
| Neovim | `<themes dir>/<theme>/neovim.lua.mustache` | `<data dir>/nvim/lua/barista.lua` (creates `nvim/lua/`) | Send `<Cmd>lua require('barista')<CR>` to every `nvim.*.0` socket under `$XDG_RUNTIME_DIR` (fallback `$TMPDIR/nvim.<user>`) via `nvim --server <socket> --remote-send` |
| Zellij | `<themes dir>/<theme>/zellij.kdl.mustache` | `<config dir>/zellij/themes/barista.kdl` (creates `zellij/themes/`) | `touch <config dir>/zellij/config.kdl` |
...
- `name` — the theme's name (the flavor's `name` string).
...
internal/theme/theme.go    # Theme type; Load/List read themes from the themes directory
internal/flavor/flavor.go  # Flavor, Palette types; Parse decodes flavor.toml content
internal/paths/paths.go    # XDG dir resolution, themes/data/config paths, filepath join
internal/template/template.go  # Mustache render wrapper
internal/recipe/recipe.go  # Recipe interface
internal/recipe/fzf/fzf.go
...
`apply` loops over the four recipes in order, calling `Run` on each and collecting errors into a slice. The render-and-write step inside each recipe stays unit-testable through package-level helper functions, not through the interface.

## Resolved decisions

- **Module layout** — `cmd/` + `internal/` split; the standard idiomatic Go CLI layout.
- **Recipe interface** — single method `Run(t Theme) error`, per Go's small-interface idiom.
...
Stdlib `testing` + `testify/assert`. The Gleam test suite is a single placeholder (`hello_world_test`), so there is no existing behavior coverage to port — we write fresh tests for the Go implementation. Priority targets: flavor parsing, theme loading, path resolution, template rendering, and the recipe render step (the reload steps shell out to external processes and are harder to unit-test; an integration test that exercises the full `apply` against a temp themes dir is the likely seam).
