...
- The interactive picker (`barista apply` with no theme argument) is a huh form, and huh's **accessible mode** is the screen-reader surface for it: the TUI is replaced by plain numbered prompts, read linearly, no in-place redraws. It is enabled with the `BARISTA_ACCESSIBLE` environment variable — set once in the user's shell, not a per-invocation flag — following huh's own recommendation to wire accessibility to an env var.
- Accessible mode happens to work over pipes and captured stdio, but with no input on stdin it silently falls back to a default selection. So it stays strictly opt-in: barista never enables it implicitly. When `apply` has no theme argument and stdin is not a terminal, it fails with a plain, complete list of available themes — that error message is the scripted and screen-reader surface, no interaction required.
